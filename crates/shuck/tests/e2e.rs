//! End-to-end integration tests for the shuck daemon.
//!
...
//!
//! Run with: `cargo test -p shuck --test e2e -- --ignored`

// ── Helpers ──────────────────────────────────────────────────────────────

/// Spawn `shuck shell <vm_name>` wrapped in a platform-appropriate PTY via `script`.
///
/// macOS: `script -q /dev/null shuck shell <vm>`
/// Linux: `script -qec "shuck shell <vm>" /dev/null`
fn spawn_shell_with_pty(vm_name: &str) -> tokio::process::Child {
    use tokio::process::Command;

    #[cfg(target_os = "macos")]
    let child = Command::new("script")
        .args(["-q", "/dev/null", "shuck", "shell", vm_name])
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()
        .expect("failed to spawn shuck shell via script");

    #[cfg(target_os = "linux")]
    let child = Command::new("script")
        .args(["-qec", &format!("shuck shell {vm_name}"), "/dev/null"])
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()
...
///
/// Requires:
/// - Running `shuck daemon` on localhost:7777
/// - Valid kernel at /var/lib/shuck/kernels/vmlinux
/// - Valid rootfs image
...
async fn vm_lifecycle() {
    ...
    let create_body = serde_json::json!({
        "name": "e2e-test",
        "kernel_path": "/var/lib/shuck/kernels/vmlinux",
        "rootfs_path": "/var/lib/shuck/images/ubuntu-22.04.ext4",
        "vcpu_count": 1,
        "mem_size_mib": 128,
    });
    let resp = client
    ...
    let write_body = serde_json::json!({
        ...
        "data": shuck_agent_proto::base64_encode(b"e2e test data"),
    });
    let resp = client
    ...
    let decoded = shuck_agent_proto::base64_decode(file_data["data"].as_str().unwrap()).unwrap();
...
async fn shell_nonexistent_vm_fails() {
    ...
    let output = Command::new("shuck")
        .args(["shell", "no-such-vm-e2e-test"])
        .output()
        .await
        .expect("failed to spawn shuck shell");
...
/// Requires:
/// - Running `shuck daemon` on localhost:7777
/// - Valid kernel at ~/.local/share/shuck/kernels/Image-virt
...
async fn vm_lifecycle_macos() {
    ...
    let data_dir = format!("{home}/.local/share/shuck");
