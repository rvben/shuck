//! API integration tests that exercise the full router with a real ShuckCore
...
use shuck_api::{VmResponse, router};
...
async fn health_counts_vms_correctly() {
    ...
    let core = make_core(state, storage, PathBuf::from("/tmp/shuck-test/run"));
...
async fn metrics_endpoint_returns_prometheus_text() {
    ...
    assert!(body.contains("shuck_api_requests_total"));
    assert!(body.contains("shuck_vms_total"));
    assert!(body.contains("shuck_api_uptime_seconds"));
}
...
async fn vm_response_json_structure() {
    ...
    let populated_core = make_core(state, storage, PathBuf::from("/tmp/shuck-test/run"));
