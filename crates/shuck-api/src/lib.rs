//! HTTP API surface for shuck, including OpenAPI docs, auth, policy, and shell/log endpoints.
...
use shuck_core::{
    ...
    ImageRecord, ImportImageRequest, RestoreSnapshotRequest, RotateSecretRequest, SecretMetadata,
    ServiceRecord, ShellEvent, ShuckCore, SnapshotRecord, VmRecord,
};
use shuck_vmm::VmmBackend;

type AppState<B> = Arc<ShuckCore<B>>;
...
pub fn router<B: VmmBackend + 'static>(core: Arc<ShuckCore<B>>) -> Router {
...
async fn metrics_handler<B: VmmBackend + 'static>(State(core): State<AppState<B>>) -> String {
    ...
    let m = metrics();
    format!(
        "# TYPE shuck_api_requests_total counter\n\
shuck_api_requests_total {}\n\
# TYPE shuck_api_errors_total counter\n\
shuck_api_errors_total {}\n\
# TYPE shuck_api_rate_limited_total counter\n\
shuck_api_rate_limited_total {}\n\
# TYPE shuck_exec_total counter\n\
shuck_exec_total {}\n\
# TYPE shuck_file_reads_total counter\n\
shuck_file_reads_total {}\n\
# TYPE shuck_file_writes_total counter\n\
shuck_file_writes_total {}\n\
# TYPE shuck_shell_sessions_total counter\n\
shuck_shell_sessions_total {}\n\
# TYPE shuck_vms_total gauge\n\
shuck_vms_total {}\n\
# TYPE shuck_vms_running gauge\n\
shuck_vms_running {}\n\
# TYPE shuck_api_uptime_seconds gauge\n\
shuck_api_uptime_seconds {}\n",
...
async fn read_file_handler<B: VmmBackend + 'static>(
    ...
    Ok(Json(ReadFileResponse {
        data: shuck_agent_proto::base64_encode(&data),
        size,
    }))
}
...
async fn write_file_handler<B: VmmBackend + 'static>(
    ...
    let data = shuck_agent_proto::base64_decode(&req.data).map_err(|_| {
        (
                    ...
                    Some(Ok(Message::Text(text))) => {
                        match serde_json::from_str::<WsShellInput>(&text) {
                            Ok(WsShellInput::Data { data }) => {
                                let bytes = match shuck_agent_proto::base64_decode(&data) {
                                    Ok(b) => b,
                                    Err(e) => {
...
fn map_error(err: CoreError) -> (StatusCode, Json<ErrorResponse>) {
    let (status, code, message) = match &err {
        ...
        CoreError::Agent(shuck_core::AgentError::NotReady(_)) => (
...
fn map_agent_connect_error(err: CoreError) -> (StatusCode, Json<ErrorResponse>) {
    match err {
        CoreError::Vmm(shuck_vmm::VmmError::VmNotFound(_))
        | CoreError::Vmm(shuck_vmm::VmmError::ProcessError(_))
        | CoreError::Vmm(shuck_vmm::VmmError::ApiError(_))
        | CoreError::Agent(shuck_core::AgentError::Connection(_))
        | CoreError::Agent(shuck_core::AgentError::VsockConnectRejected(_))
        | CoreError::Agent(shuck_core::AgentError::NotReady(_)) => (
            StatusCode::SERVICE_UNAVAILABLE,
            error_response_with_hint(
                "agent_not_ready",
...
mod tests {
    ...
    fn make_core(
        ...
        runtime_dir: PathBuf,
    ) -> Arc<ShuckCore<shuck_vmm::firecracker::FirecrackerBackend>> {
        ...
        #[cfg(not(feature = "linux-net"))]
        {
            Arc::new(ShuckCore::new(vmm, state, storage, runtime_dir))
        }
    }
    ...
    fn test_core() -> Arc<ShuckCore<shuck_vmm::firecracker::FirecrackerBackend>> {
        ...
        let storage = shuck_storage::StorageConfig {
            data_dir: PathBuf::from("/tmp/shuck-test"),
        };
        make_core(state, storage, PathBuf::from("/tmp/shuck-test/run"))
    }
    ...
    async fn snapshot_crud_basic() {
        ...
        state
            .insert_vm(&shuck_state::VmRecord {
                id: uuid::Uuid::new_v4(),
                name: "snap-vm".into(),
                state: "stopped".into(),
                pid: Some(1234),
    ...
    async fn image_crud_and_export_basic() {
        ...
        let core = make_core(
            shuck_state::StateStore::open_memory().unwrap(),
            shuck_storage::StorageConfig {
                data_dir: data_dir.clone(),
            },
            runtime_dir,
    ...
    async fn write_file_policy_denied_returns_403() {
        ...
        let body = serde_json::json!({
            "path": "/etc/passwd",
            "data": shuck_agent_proto::base64_encode(b"x"),
            "mode": null
        });
        let response = app
            .oneshot(
    ...
    async fn write_file_too_large_returns_413() {
        ...
        let body = serde_json::json!({
            "path": "/tmp/output.bin",
            "data": shuck_agent_proto::base64_encode(b"xy"),
            "mode": null
        });
        let response = app
            .oneshot(
    ...
    fn error_mapping_variants() {
        ...
        let (status, _) = map_error(CoreError::Storage(
            shuck_storage::StorageError::CommandFailed("x".into()),
        ));
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);

        let (status, _) = map_error(CoreError::State(shuck_state::StateError::LockPoisoned));
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);

        let (status, _) = map_error(CoreError::Vmm(shuck_vmm::VmmError::VmNotFound(
            uuid::Uuid::new_v4(),
        )));
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
        ...
        #[cfg(feature = "linux-net")]
        {
            let (status, _) = map_error(CoreError::Network(shuck_net::NetError::CommandFailed {
                cmd: "x".into(),
                message: "y".into(),
            }));
            assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
    ...
    fn map_agent_connect_error_returns_service_unavailable_with_hint() {
        let (status, body) = map_agent_connect_error(CoreError::Agent(
            shuck_core::AgentError::Connection(std::io::Error::other("dial failed")),
        ));
        assert_eq!(status, StatusCode::SERVICE_UNAVAILABLE);
