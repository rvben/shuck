...
impl VmmBackend for FirecrackerBackend {
    ...
    async fn create_vm(&self, config: VmConfig) -> Result<VmInfo, VmmError> {
        ...
        // Capture it to a file so `shuck logs` can read it.
...
mod tests {
    ...
    fn vm_config_serialization() {
        let config = VmConfig {
            ...
            vcpu_count: 2,
            mem_size_mib: 256,
            kernel_path: "/var/lib/shuck/kernels/vmlinux".into(),
            rootfs_path: "/var/lib/shuck/images/ubuntu.ext4".into(),
            kernel_args: None,
            initrd_path: None,
            vsock_cid: 3,
            tap_device: Some("shuck3".into()),
        ...
        assert_eq!(json["tap_device"], "shuck3");
    }
