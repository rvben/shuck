...
async fn main() -> Result<()> {
    ...
    info!("shuck-agent starting");

    // Transport selection:
    // 1. SHUCK_AGENT_SOCKET env var → Unix socket (dev/testing)
    // 2. Linux → vsock port 52 (production)
    // 3. macOS → default Unix socket fallback (dev)
    if let Ok(path) = std::env::var("SHUCK_AGENT_SOCKET") {
        listen_unix(&path).await
    } else if cfg!(target_os = "linux") {
        listen_vsock().await
    } else {
        let default_path = "/tmp/shuck-agent.sock";
...
async fn listen_unix(path: &str) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = shuck_agent::handle_connection(stream).await {
...
async fn listen_vsock() -> Result<()> {
    ...
    let port = shuck_agent_proto::AGENT_VSOCK_PORT;
...
async fn listen_vsock() -> Result<()> {
    anyhow::bail!("vsock is only available on Linux; set SHUCK_AGENT_SOCKET for dev use")
}
