...
//!
//!   cargo test -p shuck-net --test e2e_bridge -- --ignored --test-threads=1
//!
//! `--test-threads=1` is required because tests share the global `shuck`
//! nftables table.
//!
...
fn nft_table_exists() -> bool {
    Command::new("nft")
        .args(["list", "table", "ip", "shuck"])
        .output()
        .is_ok_and(|o| o.status.success())
}

fn nft_table_output() -> String {
    cmd_output("nft", &["list", "table", "ip", "shuck"])
}
...
async fn bridge_create_and_delete() {
    let bridge = "shucktest0";

    // Clean up from any prior failed run
    let _ = shuck_net::delete_bridge(bridge).await;

    // Create bridge
    shuck_net::create_bridge(bridge, Ipv4Addr::new(10, 99, 0, 1), 24)
        .await
    ...
    // Delete bridge
    shuck_net::delete_bridge(bridge)
        .await
...
async fn tap_create_and_delete() {
    ...
    // Clean up from any prior failed run
    let _ = shuck_net::delete_tap(tap).await;

    shuck_net::create_tap(tap)
        .await
...
async fn tap_attaches_to_bridge() {
    ...
    shuck_net::create_bridge(bridge, Ipv4Addr::new(10, 99, 1, 1), 24)
        .await
        .expect("create_bridge");
    shuck_net::create_tap(tap).await.expect("create_tap");

    // Attach TAP to bridge
    shuck_net::attach_to_bridge(tap, bridge)
        .await
    ...
    shuck_net::delete_tap(tap).await.expect("delete_tap");
...
async fn multiple_taps_on_bridge() {
    let bridge = "shucktest4";
    let taps = ["shucktst5", "shucktst6", "shucktst7"];
    ...
    shuck_net::create_bridge(bridge, Ipv4Addr::new(10, 99, 2, 1), 24)
        .await
        .expect("create_bridge");
    ...
    shuck_net::delete_tap(taps[1]).await.expect("delete_tap");
    assert!(interface_has_master(taps[0], bridge));
    assert!(!interface_exists(taps[1]));
    assert!(interface_has_master(taps[2], bridge));

    // Cleanup
    shuck_net::delete_tap(taps[0]).await.expect("delete_tap");
    shuck_net::delete_tap(taps[2]).await.expect("delete_tap");
    shuck_net::delete_bridge(bridge)
        .await
        .expect("delete_bridge");
}
...
async fn nftables_init_and_cleanup() {
    ...
    shuck_net::init_nat("shucktest0", "10.99.0.0/24", "eth0")
        .await
        .expect("init_nat should succeed");

    assert!(nft_table_exists(), "shuck nftables table should exist");
    ...
    assert!(
        output.contains("shuck:bridge-masq"),
        "masquerade rule should have bridge-masq comment"
    );
    assert!(
        output.contains("shuck:bridge-fwd-out"),
        "should have forward-out rule"
    );
    assert!(
        output.contains("shuck:bridge-fwd-in"),
    ...
    // Cleanup
    shuck_net::cleanup_nat().await.expect("cleanup_nat");
    assert!(
        !nft_table_exists(),
        "shuck table should be gone after cleanup"
    );
}
...
async fn port_forward_add_and_remove() {
    ...
    // Add a port forward
    shuck_net::add_port_forward(8080, Ipv4Addr::new(10, 99, 0, 2), 80, "shucktst1")
        .await
    ...
    assert!(
        output.contains("shuck-pf:shucktst1:8080"),
        "DNAT should have comment tag"
    );

    // Add a second port forward
    shuck_net::add_port_forward(9090, Ipv4Addr::new(10, 99, 0, 3), 443, "shucktst2")
        .await
        .expect("add_port_forward 2");

    // Remove first port forward
    shuck_net::remove_port_forward(8080, "shucktst1")
        .await
    ...
    assert!(
        output.contains("shuck-pf:shucktst2:9090"),
        "second port forward should remain"
    );

    // Remove all port forwards for shucktst2
    shuck_net::remove_all_port_forwards("shucktst2")
        .await
        .expect("remove_all_port_forwards");

    let output = nft_table_output();
    assert!(
        !output.contains("shuck-pf:shucktst2"),
        "all shucktst2 port forwards should be removed"
    );
...
async fn full_lifecycle_bridge_tap_nat() {
    let bridge = "shucktst8";

    // Cleanup from prior runs
    let _ = shuck_net::cleanup_nat().await;
    let _ = shuck_net::delete_tap("shucktst9").await;
    let _ = shuck_net::delete_tap("hskts10").await;
    let _ = shuck_net::delete_bridge(bridge).await;

    // 1. Create allocator
    let alloc = shuck_net::IpAllocator::new(Ipv4Addr::new(10, 99, 3, 0), 24);
    let gateway = alloc.gateway();
    let prefix_len = alloc.prefix_len();
    assert_eq!(gateway, Ipv4Addr::new(10, 99, 3, 1));

    // 2. Create bridge
    shuck_net::create_bridge(bridge, gateway, prefix_len)
        .await
        .expect("create_bridge");
    ...
    // 3. Init NAT
    shuck_net::init_nat(bridge, "10.99.3.0/24", "eth0")
        .await
        .expect("init_nat");
    ...
    let tap1 = "shucktst9";
    shuck_net::create_tap(tap1).await.expect("create_tap vm1");
    shuck_net::attach_to_bridge(tap1, bridge)
        .await
        .expect("attach vm1");

    // Verify kernel args would be correct
    let netmask = shuck_net::prefix_len_to_netmask(prefix_len);
    ...
    let tap2 = "hskts10";
    shuck_net::create_tap(tap2).await.expect("create_tap vm2");
    shuck_net::attach_to_bridge(tap2, bridge)
        .await
        .expect("attach vm2");
    ...
    // 6. Add port forwards
    shuck_net::add_port_forward(2222, vm1_ip, 22, tap1)
        .await
        .expect("add pf vm1");
    shuck_net::add_port_forward(2223, vm2_ip, 22, tap2)
        .await
        .expect("add pf vm2");

    let nft_out = nft_table_output();
    assert!(nft_out.contains("shuck-pf:shucktst9:2222"));
    assert!(nft_out.contains("shuck-pf:hskts10:2223"));

    // 7. Destroy VM 1
    shuck_net::remove_all_port_forwards(tap1)
        .await
        .expect("remove pf vm1");
    shuck_net::delete_tap(tap1).await.expect("delete tap vm1");
    ...
    shuck_net::remove_all_port_forwards(tap2)
        .await
        .expect("remove pf vm2");
    shuck_net::delete_tap(tap2).await.expect("delete tap vm2");
...
async fn init_nat_is_idempotent() {
    ...
    let output = nft_table_output();
    let masq_count = output.matches("shuck:bridge-masq").count();
    assert_eq!(
        masq_count, 1,
        "should have exactly one masquerade rule, got {masq_count}"
    );

    shuck_net::cleanup_nat().await.expect("cleanup");
}
