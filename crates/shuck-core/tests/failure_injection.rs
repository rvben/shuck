...
fn core_with_vm(name: &str, state: &str, fail_ops: &[&'static str]) -> Arc<ShuckCore<FailingVmm>> {
    ...
    let storage = shuck_storage::StorageConfig {
        data_dir: PathBuf::from("/tmp/shuck-failure-test"),
    };
    ...
    #[cfg(feature = "linux-net")]
    {
        Arc::new(ShuckCore::new(
            ...
            PathBuf::from("/tmp/shuck-failure-test/run"),
        ))
    }
