...
|---|---|---|---|---|
| FC-001 | Add Firecracker `initrd_path` wiring in `/boot-source` payload and cover with integration tests | P1 | Done | Commit `5efa1b3`, `cargo test -p shuck-vmm` |
| FC-002 | Make `add_port_forward` transactional: rollback nft rule if DB insert fails, or persist first then apply with compensation | P1 | Done | Commit `1fd8f61`, `cargo test -p shuck-core` |
| FC-003 | Map transient agent-connect failures during boot to 503 (not 500), and keep mapping deterministic | P1 | Done | Commit `b44f38c`, `cargo test -p shuck-api` |
| FC-004 | Add startup reconciler for nftables rules vs `port_forwards` table drift | P2 | Done | Commit `ded9179`, `cargo test -p shuck-core` |
| FC-005 | Define and enforce idempotency behavior for stop/pause/resume/destroy endpoints | P2 | Done | Commit `00cb6b8`, `cargo test -p shuck-core -p shuck-api` |
| FC-006 | Add property-style lifecycle tests for random valid/invalid state transitions | P2 | Done | Commit `ded9179`, `cargo test -p shuck-core --test state_transitions` |
